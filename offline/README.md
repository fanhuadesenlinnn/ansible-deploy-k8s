...
默认输出位于 `offline/bundles/`，包括离线目录、HTTP 分发使用的 tar.gz 和归档 SHA-256。构建过程先写入系统临时
...
│   └── 所有必需的 Debian 软件包依赖
├── images/
│   ├── registry.k8s.io_kube-apiserver_*.tar
│   ├── registry.k8s.io_pause_*.tar
│   └── ghcr.io_flannel-io_flannel_*.tar
└── manifests/
    └── kube-flannel.yml
```
...
创建归档。安装程序会将所有 `images/*.tar` 文件导入 `k8s.io` 命名空间。自动制作时，每个镜像会生成
一个带完整引用标签的 OCI 归档；除 kubeadm 所列镜像外，还会包含目标发行版的 containerd 实际使用的
Pod sandbox 镜像。
...
Docker 执行环境只能访问仓库目录内的文件，因此离线包应保存在 `offline/bundles/` 或其他仓库子目录中。
...
可选的 `ansible/playbooks/artifact-server.yml` 会在第一台控制平面主机上安装 dufs。它只是辅助工具，并非 Kubernetes
各 Role 的依赖。
